...
async function parseImplIndex(
	implIndexPath: string,
): Promise<{ content: string; variants: VariantInfo[]; importMap: Record<string, string> }> {
...
function removeImport(content: string, funcName: string, fileBase?: string): string {
	const re = new RegExp(
		`^.*import\\s*\\{\\s*${funcName}\\s*\\}\\s*from\\s*\\"\\.\\/variants\\/${fileBase ?? "[A-Za-z0-9_-]+"}\\";?\\r?\\n`,
		"m",
	)
...
async function main() {
	...
	const parsed = await parseImplIndex(implIndexPath)
